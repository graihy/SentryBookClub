...
namespace BookClub.Entities
{
    public static class LoggerDefines
    {
        ...
        {
            _repoGetBooks = LoggerMessage.Define(LogLevel.Information, 0, "Inside the repository about to call GetAllBooks.");

            _repoCallGetManyProc = LoggerMessage.Define<string>(LogLevel.Debug, DataEvents.GetMany, "Debugging information for stored proc: {ProcName}");

            _apiGetAllBooksScope = LoggerMessage.DefineScope<string>("Constructing books response for {ScopedUserId}");
        }
